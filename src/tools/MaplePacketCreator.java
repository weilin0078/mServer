...
public class MaplePacketCreator
{
    ...
    public static MaplePacket showmesoRanks(final int npcid, final List<MapleGuildRanking.mesoRankingInfo> all) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
        mplew.write(73);
        mplew.writeInt(npcid);
        mplew.writeInt(all.size());
        for (final MapleGuildRanking.mesoRankingInfo info : all) {
            mplew.writeMapleAsciiString(info.getName());
    ...
    public static final MaplePacket getNodeProperties(final MapleMonster objectid, final MapleMap map) {
        ...
        for (final MapleNodes.MapleNodeInfo mni : map.getNodes()) {
