    ...
    public static final void DueyOperation(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (operation) {
            case 1: {
                ...
                if (conv == 2) {
                    c.getSession().write((Object)MaplePacketCreator.sendDuey((byte)10, loadItems(c.getPlayer())));
                    break;
                }
                break;
            }
    ...
    private static final MapleDueyActions getItemByPID(final int packageid) {
        try {
            ...
            if (iter != null && iter.size() > 0) {
                final Iterator i$ = iter.values().iterator();
                if (i$.hasNext()) {
                    final Pair<IItem, MapleInventoryType> i = (Pair)i$.next();
