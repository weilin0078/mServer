...
public class StatsHandling
{
    ...
    public static final void AutoAssignAP(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        for (int count = slea.readInt(), i = 0; i < count; ++i) {
            ...
            if (chr.getRemainingAp() >= updatenumber) {
                ...
                StatsHandling.log.info("[h4x] Player {} is distributing AP to {} without having any", (Object)chr.getName(), (Object)update);
            }
        }
        statupdate.add(new Pair<MapleStat, Integer>(MapleStat.AVAILABLEAP, (int)chr.getRemainingAp()));
        c.getSession().write((Object)MaplePacketCreator.updatePlayerStats(statupdate, true, chr.getJob()));
    ...
    static {
        StatsHandling.log = LoggerFactory.getLogger((Class)StatsHandling.class);
    }
}
