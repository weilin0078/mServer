...
public class PlayerStats implements Serializable
{
    ...
    private Map<Integer, Integer> setHandling;
    private List<Equip> durabilityHandling;
    private List<Equip> equipLevelHandling;
    ...
    public void recalcLocalStats(final boolean first_login) {
        ...
        for (final IItem item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
            ...
            if (equip.getState() > 1) {
                final int[] arr$2;
                final int[] potentials = arr$2 = new int[] { equip.getPotential1(), equip.getPotential2(), equip.getPotential3() };
                for (final int i : arr$2) {
                    if (i > 0) {
                        final StructPotentialItem pot = ii.getPotentialInfo(i).get(ii.getReqLevel(equip.getItemId()) / 10);
                        if (pot != null) {
                            ...
                            if (!pot.boss) {
                                this.dam_r = Math.max(pot.incDAMr, this.dam_r);
                            }
                            else {
                                this.bossdam_r = Math.max(pot.incDAMr, this.bossdam_r);
                            }
                ...
                if (GameConstants.getStatFromWeapon(equip.getItemId()) == null) {
                    if (equip.getEquipLevels() > GameConstants.getMaxLevel(equip.getItemId())) {
                        continue;
                    }
                }
                else if (equip.getEquipLevels() >= GameConstants.getMaxLevel(equip.getItemId())) {
                    continue;
                }
