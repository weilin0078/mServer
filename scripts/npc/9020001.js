...
function getPrize(eim,cm) {
	...
	//var renwu = Array(4032491,4032492,4032493,4032494,4032495);
	//var rw_random = Math.floor(Math.random() * 5);
	//cm.gainItem(renwu[rw_random], 1);
    ...
    var targetPortal = target.getPortal("st00");
	//cm.worldMessage(6,"[废弃组队]:" + cm.getName()+ "  老司机开车,通过了废弃组队副本！~");
	cm.喇叭2(1,"废弃副本","成功通关[组队任务 - 废弃组队]副本,获得大量奖励.");
    cm.getPlayer().changeMap(target, targetPortal);
}