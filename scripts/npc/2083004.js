   ...
   /* if (cm.getPlayer().getClient().getChannel() != 4 && cm.getPlayer().getClient().getChannel() != 2) {
    ...
    if (prop == null || prop.equals("0")) {
        ...
        if (squadAvailability == -1) {
            status = 0;
			if (cm.getBossLog("普通黑龙") >= maxHorntail && !cm.getPlayer().isGM()) {
				cm.sendOk("您今天挑战暗黑龙王的次数已经用完，请明天在来挑战吧！");
				cm.dispose();
    	     }else if(cm.checkPartyBossLog("普通黑龙",maxHorntail) && !cm.getPlayer().isGM()){
                cm.sendOk("您队伍有人今天挑战暗黑龙王的次数已经用完，请让他明天在来挑战吧！");
                cm.dispose();
                return;
             }
            ...
            if (eim == null) {
                ...
                if (squd != null) {
					...
					} */ if (cm.getBossLog('heilong') >= maxHorntail && !cm.getPlayer().isGM()) {
						cm.sendOk("您今天挑战暗黑龙王的次数已经用完，请明天在来挑战吧！");
						cm.dispose();
    	            } else if(cm.checkPartyBossLog("heilong",maxHorntail) && !cm.getPlayer().isGM()){
                        cm.sendOk("您队伍有人今天挑战暗黑龙王的次数已经用完，请让他明天在来挑战吧！");
                        cm.dispose();
                        return;
                    }
        ...
        if (mode == 1) {
            if (selection == 0) {
                ...
                if (cm.getSquad("Horntail") != null) {
                    ...
                    cm.givePartyBossLog("普通黑龙");
					cm.givePartyBossBack("黑龙");//BOSS重返
