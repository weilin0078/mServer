...
function action(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 0) {
            cm.sendOk("不想使用吗？…我的肚子里有各类#b技能书#k哦！");
            cm.dispose();
        }
        status--;
    }
    if (status == 0) {
        if (cm.haveItem(4001084)) {
            cm.sendYesNo("冒险岛转蛋机中有各类#b技能书#k噢！使用“#b#t4001084##k”。 假如没有象征的话，是不可以使用我的。现在要玩转蛋机么? ");
        } else {
        ...
        if (finalitem.length != 0) {
            ...
            item = cm.gainGachaponItem(itemId, quantity, "抽取技能书", notice);
            if (item != -1) {
                cm.gainItem(4001084, -1);
                cm.sendOk("你获得了 #b#t" + item + "##k " + quantity + "个。");
            } else {
                cm.sendOk("你确实有#b#t4001084##k吗？如果是，请你确认在背包的装备，消耗，其他窗口中是否有一格以上的空间。");
            }
            cm.safeDispose();
        } else {
